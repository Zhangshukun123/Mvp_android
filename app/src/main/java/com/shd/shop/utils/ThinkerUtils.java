...
public class ThinkerUtils {
    ...
    public static int checkForPatchRecover(long roomSize, int maxMemory) {
        if (ThinkerUtils.isGooglePlay()) {
            return ThinkerUtils.ERROR_PATCH_GOOGLEPLAY_CHANNEL;
        }
        if (maxMemory < MIN_MEMORY_HEAP_SIZE) {
            return ThinkerUtils.ERROR_PATCH_MEMORY_LIMIT;
        }
        //or you can mention user to clean their rom space!
        if (!checkRomSpaceEnough(roomSize)) {
            return ThinkerUtils.ERROR_PATCH_ROM_SPACE;
        }
